...
// shouldn't be used
text_message_class::text_message_class():message_class(message_type_enum::T_undefined_message){
    std::cout <<"text message class: default constructor" << std::endl;
    std::cout << "id = " << get_id() << " , data length = " << get_data_length() << std::endl;
    print_header_data();
}

text_message_class::text_message_class(std::string text):message_class(message_type_enum::T_other_message){
